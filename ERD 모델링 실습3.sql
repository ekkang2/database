...
#데이터 입력
insert into `Users` (`userId`, `userName`, `userBirth`, `userGender`, `userHp`, `userEmail`, `userPoint`, `userLevel`, `userAddr`, `userRegDate`) values ('user1','김유신','1976-01-21','M','010-1101-1976','kimys@naver.com', 0, 1,'서울','2022-01-10 10:50:12');
...
insert into `Users` (`userId`, `userName`, `userBirth`, `userGender`, `userHp`, `userEmail`, `userPoint`, `userLevel`, `userAddr`, `userRegDate`) values ('user4','이사부','1979-04-13','M','010-2101-1979','leesabu@gmail.com', 2600, 1,'서울','2022-01-10 10:50:12');
insert into `Users` (`userId`, `userName`, `userBirth`, `userGender`, `userHp`, `userEmail`, `userPoint`, `userLevel`, `userAddr`, `userRegDate`) values ('user5','장보고','1966-09-12','M','010-2104-1966','jangbg@naver.com', 9400, 4,'대전','2022-01-10 10:50:12');
insert into `Users` (`userId`, `userName`, `userBirth`, `userGender`, `userHp`, `userEmail`, `userPoint`, `userLevel`, `userAddr`, `userRegDate`) values ('user6','선덕여왕','1979-07-28','M','010-3101-1979','gueen@naver.com', 1600, 2,'대전','2022-01-10 10:50:12');
insert into `Users` (`userId`, `userName`, `userBirth`, `userGender`, `userHp`, `userEmail`, `userPoint`, `userLevel`, `userAddr`, `userRegDate`) values ('user7','강감찬','1984-06-15','F','010-4101-1984','kang@daum.net', 800, 0,'대구','2022-01-10 10:50:12');
insert into `Users` (`userId`, `userName`, `userBirth`, `userGender`, `userHp`, `userEmail`, `userPoint`, `userLevel`, `userAddr`, `userRegDate`) values ('user8','신사임당','1965-10-21','M','010-5101-1965','sinsa@gmail.com', 1500, 1,'대구','2022-01-10 10:50:12');
insert into `Users` (`userId`, `userName`, `userBirth`, `userGender`, `userHp`, `userEmail`, `userPoint`, `userLevel`, `userAddr`, `userRegDate`) values ('user9','이이','1972-11-28','F','010-6101-1972','leelee@nate.com', 3400, 3,'부산','2022-01-10 10:50:12');
...
select `userName`, `prodName`, `cartProdCount` 
  from `users` as a
  join `carts` as b on b.userId = a.userId
  join `products` as c on c.prodNo = b.prodNo
...
select `prodNo`, `cateName`, `prodName`, `prodPrice`, `sellerManager`, `sellerPhone`
  from `products` as a
  join `sellers` as b on b.sellerNo = a.sellerNo
  join `categories` as c on c.cateNo = a.cateNo;


#문제3. 모든 고객의 아이디, 이름, 휴대폰, 현재포인트, 적립포인트 총합을 조회하시오. 단 적립포인트 내역이 없으면 0으로 출력
select a.userId, a.userName, a.userHp, userPoint, ifnull(sum(b.point), 0) as `총합`
  from `users` as a
  left join `points` as b on b.userId = a.userId
  group by a.userId;
  
#문제4. 모든 주문의 주문번호, 주문자 아이디, 고객명, 주문가격, 주문일자를 조회하시오. 단 주문금액에 10만원 이상, 큰 금액순으로 조회, 금액이 같으면 이름이 사전순으로 될것
select a.orderNo, b.userId, b.userName, c.itemPrice, a.orderDate 
  from `orders` as a
  join `users` as b on b.userId = a.userId
  join `orderitems` as c on c.orderNo = a.orderNo
  where c.itemPrice >= 100000
  order by c.itemPrice asc, userName desc;

#문제5. 모든 주문의 주문번호, 주문자 아이디, 고객명, 상품명, 주문일자를 조회하시오. 주문번호는 중복없이 상품명은 구분자 ,로 나열할것
select b.orderNo, a.userId, a.userName, GROUP_CONCAT(d.prodName SEPARATOR ',') AS `상품명`, b.orderDate
  from `users` as a
  join `orders` as b on b.userId = a.userId
...
select a.sellerNo, a.sellerBizName, a.sellerManager, a.sellerPhone
  from `sellers` as a
left join `products` as b on b.sellerNo = a.sellerNo;

#문제9. 모든 주문상세내역 중 개별 상품 가격과 개수 그리고 할인율이 적용된 가격을 구하고 그 가격으로 주문별 총합을 구해서 주문별 총합이 10만원이상 그리고 큰 금액 순으로 '주문번호', '최종총합'을 조회하시오.

#문제10. 장보고 고객이 주문했던 모든 상품명을 '고객명', '상품명'으로 조회하시오. 단 상품명은 중복안됨, 상품명은 구분자 ,로 나열
select a.userName as `고객명` , GROUP_CONCAT(d.prodName SEPARATOR ',') AS `상품명` 
  from `users` as a
  join `orders` as b on b.userId = a.userId
  join `orderitems` as c on c.orderNo = b.orderNo
  join `products` as d on d.prodNo = c.prodNo
