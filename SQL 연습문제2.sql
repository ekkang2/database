...
#실습 2-12
select * from `bank_customer` where `c_name` like '(주)%';
...
#실습 2-20
select count(case when `t_dist` = 1 then `t_dist` end) as `입금 건수`,
...
select `t_dist`, 
	case
		when `t_dist` = 1 then `t_dist` 
		when `t_dist` = 2 then `t_dist`
		when `t_dist` = 3 then `t_dist`
    end as 'type',
    `t_a_no`,
    `t_amount`
from `bank_transaction`;
...
left outer join `bank_account` as b
 on b.a_no = a.t_a_no
left outer join `bank_customer` as c
 on c.c_no = b.a_c_no
where `t_dist` = 1
